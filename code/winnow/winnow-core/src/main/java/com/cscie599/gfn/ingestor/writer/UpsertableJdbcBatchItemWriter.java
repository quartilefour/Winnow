...
public class UpsertableJdbcBatchItemWriter<T> extends JdbcBatchItemWriter<T> {
    ...
    public void write(final List<? extends T> items) throws Exception {
        ...
        if (!items.isEmpty()) {
            ...
            if (JdbcBatchItemWriter.logger.isDebugEnabled()) {
                JdbcBatchItemWriter.logger.debug("Executing batch with " + items.size() + " items.");
            }
            ...
            if (assertUpdates) {
                for (int i = 0; i < updateCounts.length; i++) {
                    ...
                    if (value == 0) {
                        if(JdbcBatchItemWriter.logger.isDebugEnabled()) {
                            JdbcBatchItemWriter.logger.debug("Skipping retry as this is expected for upserts");
                        }
                    }
                }
            }
